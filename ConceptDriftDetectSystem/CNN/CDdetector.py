...
EVAL_DATA_QUANTITY = 4                       # 평가를 위한 데이터 수
...
###########################################################################
    ...
    print("CD 검출을 시작합니다.\n입력 윈도우의 수는 %d이며, 임계값은 %f인 상태로 진행합니다."%(BATCH_QUANTITY_FOR_EVAL, THRESHOLD))
    ...
    vectorA = sess.run(CNN.y_conv_wo_softmax, feed_dict={
    ...
    for i in range(1, BATCH_QUANTITY_FOR_EVAL):                                # 1개는 이미 받아왔으므로
        ...
        vectorB = sess.run(CNN.y_conv_wo_softmax, feed_dict={
            CNN.x: batch_values, CNN.y_:batch_labels, CNN.keep_prob: 1.0})
        
        difference = np.abs(np.subtract(vectorA, vectorB)).sum() / 101              # 두 벡터의 차이 벡터를 계산하고, 각 원소의 절대값을 모두 더함, 101은 레이블 수

        #difference = 1 - distance.cosine(vectorA, vectorB)
    ...
    print("%d 개의 윈도우 중, %d회의 concept drift가 발생하였습니다."%(EVAL_DATA_QUANTITY, drift_count))
