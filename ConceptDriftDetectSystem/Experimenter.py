...
class Experimenter:
    ...
    def result(self):
        ...
        currect_alarm = np.array(self.detect_counter[1:])
        np.putmask(currect_alarm, currect_alarm > 0, 1)
        currect_alarm = sum(currect_alarm)
        print("Currect Alarm:", currect_alarm)

        false_alarm = sum(self.detect_counter) - currect_alarm
        print("False Alarm:", false_alarm)

        false_dismissal = self.detect_counter[1:].count(0)
        print("False Dismissal:", false_dismissal)

        precision = currect_alarm / (currect_alarm + false_alarm)
        print("Precision:", precision)

        recall = currect_alarm / (currect_alarm + false_dismissal)
        print("Recall:", recall)
