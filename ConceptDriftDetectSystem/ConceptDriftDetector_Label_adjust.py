...
with tf.Session(config=config) as sess:
    ...
    reference_window.label = sess.run(CNN.top_k_op, feed_dict={
            CNN.x: [reference_window.data], CNN.y_: [[0 for x in range(300)]], CNN.keep_prob: 1.0})[1][0][0]      # 최초 레퍼런스 윈도우에 대한 레이블을 설정
    ...
    while(True):    # ::::::::::::::조건 나중에 변경해야 할 것임. 스트림의 끝을 어떻게 감지하지?::::::::::::::
        ...
        dst_window.label = sess.run(CNN.top_k_op, feed_dict={
            CNN.x: [dst_window.data], CNN.y_: [[0 for x in range(300)]], CNN.keep_prob: 1.0})[1][0][0]      # 비교 대상 윈도우에 대한 레이블을 설정
        ...
        if (difference_matrix[reference_window.label][dst_window.label] > THRESHOLD_TO_TRIGGER):
            ...
            print(dst_window.time, "시점에서 Concept drift가 발생했습니다.(", difference_matrix[reference_window.label][dst_window.label],")")
