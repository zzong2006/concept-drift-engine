...
def getFromDB(a_json):
    ...
    if (a_json['message'] == 'new-src'):
        ...
        sql4 = 'select * from tbl_src where NAME="' + str(a_json['src-name']) + '" and ' + 'F_OWNER="' + str(user_idx) + '"'
        curs.execute(sql4)
        ...
        sql5 = 'select * from tbl_src_test_data where IDX="' + str(f_test_data) + '" and ' + 'F_OWNER="' + str(user_idx) + '"'
        curs.execute(sql5)
    ...
    if (a_json['message'] == 'activate-src') :
        ...
        sql6 = 'select * from tbl_src where NAME="' + str(a_json['src-name']) + '" and ' + 'F_OWNER="' + str(user_idx) + '"'
        curs.execute(sql6)
...
def change_status(a_json, result):
    ...
    try :
        ...
        sql = "UPDATE tbl_src SET CONCEPT_DRIFT_STATUS='%s' WHERE NAME='%s' and F_OWNER='%s'" % (query, str(a_json['src-name']), str(user_idx))
        logSql = "INSERT INTO tbl_log(F_USER,LOGGING_TYPE,LOGGING_MESSAGE) VALUES({},{},{})".format(user_idx, '"INFO"',
                '"[CONCEPT DRIFT]Model of source({}) status is changed to {}"'.format(a_json['src-name'], query))
...
def sendLog(a_json, logtype):
    ...
    try :
        ...
        if logtype == 1:
            logSql = "INSERT INTO tbl_log(F_USER,LOGGING_TYPE,LOGGING_MESSAGE) VALUES({},{},{})".format(user_idx,
                        '"INFO"','"[CONCEPT DRIFT]Model of source({}) is generated."'.format(a_json[ 'src-name']))
        elif logtype == 2:
            logSql = "INSERT INTO tbl_log(F_USER,LOGGING_TYPE,LOGGING_MESSAGE) VALUES({},{},{})".format(user_idx,
                        '"INFO"','"[CONCEPT DRIFT]Concept Drift Detection (Source: {}) is activated."'.format(a_json[ 'src-name']))
        elif logtype == 3:
            logSql = "INSERT INTO tbl_log(F_USER,LOGGING_TYPE,LOGGING_MESSAGE) VALUES({},{},{})".format(user_idx,
                       '"INFO"','"[CONCEPT DRIFT]Concept Drift Detection (Source: {}) is stopped."'.format(a_json[ 'src-name']))
        elif logtype == 4:
            logSql = "INSERT INTO tbl_log(F_USER,LOGGING_TYPE,LOGGING_MESSAGE) VALUES({},{},{})".format(user_idx,
                      '"INFO"', '"[CONCEPT DRIFT]Model of source({}) is deleted."'.format(a_json['src-name']))
...
def determine_n_proc(a_json):
    ...
    # 데이터 스트림 읽기: 데이터 스트림에서 타겟 컬럼을 추출하여 컨셉 드리프트를 분석한다.
        ...
        sendLog(a_json, 2)
        realValue, topic_name = getFromDB(a_json)
        result = MD.setKafkaTopic(a_json['user-id'], a_json['src-name'], topic_name, realValue)
        if result != -1 :
    ...
    # 데이터 스트림 읽기 중단: 데이터 스트림을 읽지 않고 컨셉 드리프트 분석을 중단한다.
        ...
        MD.deleteModel(a_json['user-id'] + "." + a_json['src-name'])
        sendLog(a_json, 4)
